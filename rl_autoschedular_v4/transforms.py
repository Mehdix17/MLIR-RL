...
def transform_pad(code: str, operation_tag: str, padding_dimensions: list[int], pad_to_multiple_of: list[int]):
    """Apply the pad transformation to the specified operation in the given code.

    Args:
        code (str): The code to apply the transformation to.
        operation_tag (str): The tag of the operation to apply the transformation to.
        padding_dimensions (list[int]): The dimensions to pad.
        pad_to_multiple_of (list[int]): The multiple to pad each dimension to.

    Returns:
        str: The code after applying the transformation.
    """
    if not padding_dimensions or all(m == 0 for m in pad_to_multiple_of):
        return code

    # Build padding_values as array of zeros (constant 0.0 padding)
    padding_values = [0.0] * len(padding_dimensions)

    transform_code = (
        f'module attributes {{transform.with_named_sequence}} {{\n'
        f'  transform.named_sequence @__transform_main(%arg1: !transform.any_op {{transform.readonly}}) {{\n'
        f'    %op_{operation_tag} = transform.structured.match attributes{{tag = "{operation_tag}"}} in %arg1 : (!transform.any_op) -> !transform.any_op\n'
        f'    %padded_op, %pad_op, %copy_op = transform.structured.pad %op_{operation_tag} pad_to_multiple_of {str(pad_to_multiple_of)} {{padding_dimensions = {str(padding_dimensions)}}} : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op)\n'
        f'    %pad_tag = transform.param.constant "{operation_tag}" -> !transform.any_param\n'
        f'    transform.annotate %padded_op "tag" = %pad_tag : !transform.any_op, !transform.any_param\n'
        f'    transform.yield\n'
        f'  }}\n'
        f'}}\n'
    )
...
def transform_pack(code: str, operation_tag: str, packed_sizes: list[int]):
    """Apply the pack transformation to the specified operation in the given code.

    Args:
        code (str): The code to apply the transformation to.
        operation_tag (str): The tag of the operation to apply the transformation to.
        packed_sizes (list[int]): The packed sizes for each iterator dimension.
    ...
    transform_code = (
        ...
        f'  transform.named_sequence @__transform_main(%arg1: !transform.any_op {{transform.readonly}}) {{\n'
        f'    %op_{operation_tag} = transform.structured.match attributes{{tag = "{operation_tag}"}} in %arg1 : (!transform.any_op) -> !transform.any_op\n'
        f'    %packed_op = transform.structured.pack %op_{operation_tag} packed_sizes = {str(packed_sizes)} : (!transform.any_op) -> (!transform.any_op)\n'
        f'    %pack_tag = transform.param.constant "{operation_tag}" -> !transform.any_param\n'
        f'    transform.annotate %packed_op "tag" = %pack_tag : !transform.any_op, !transform.any_param\n'
        f'    transform.yield\n'
        f'  }}\n'
        f'}}\n'
    )
...
def transform_unroll(code: str, operation_tag: str, tile_sizes: list[int], factor: int):
    ...
    This first tiles the operation using structured.tile_using_for with the given
    tile sizes, then unrolls each resulting loop with the given factor. Because
    unrolling duplicates the tagged operation, subsequent tag-based transforms may
    match multiple ops, so this action should be treated as terminal.

    Args:
        code (str): The code to apply the transformation to.
        operation_tag (str): The tag of the operation to apply the transformation to.
        tile_sizes (list[int]): Per-dimension tile sizes (0 means don't tile that dim).
    ...
    num_tiled_dims = sum(1 for ts in tile_sizes if ts > 0)
    if num_tiled_dims == 0:
        return code

    loop_handles = ', '.join(f'%loop_{operation_tag}_{i}' for i in range(num_tiled_dims))
    result_types = ', '.join(['!transform.any_op'] * (num_tiled_dims + 1))

    transform_code = (
        f'module attributes {{transform.with_named_sequence}} {{\n'
        f'  transform.named_sequence @__transform_main(%arg1: !transform.any_op {{transform.readonly}}) {{\n'
        f'    %op_{operation_tag} = transform.structured.match attributes{{tag = "{operation_tag}"}} in %arg1 : (!transform.any_op) -> !transform.any_op\n'
        f'    %tiled_{operation_tag}, {loop_handles} = transform.structured.tile_using_for %op_{operation_tag} tile_sizes {str(tile_sizes)} : (!transform.any_op) -> ({result_types})\n'
    )
    for i in reversed(range(num_tiled_dims)):
        transform_code += f'    transform.loop.unroll %loop_{operation_tag}_{i} {{factor = {factor} : i64}} : !transform.any_op\n'
    transform_code += (
        f'    transform.yield\n'
        f'  }}\n'
        f'}}\n'
