...
from rl_autoschedular_v4.actions import ActionSpace, Interchange
from rl_autoschedular_v4.observation import OpFeatures, ActionHistory, ProducerOpFeatures, Observation, NumLoops, HardwareFeatures
from rl_autoschedular_v4.state import OperationType
...
class TransformerEmbedding(nn.Module):
    ...
    def forward(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_action_history_token:
            ...
            return torch.cat((pooled, hardware_feats), dim=1)

        hardware_feats = Observation.get_part(obs, HardwareFeatures)
        return torch.cat((pooled, action_history, hardware_feats), dim=1)
