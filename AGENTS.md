...
The evaluation dashboard (`dashboard/`) is a multi-page Streamlit app for comparing RL agent performance. It reads clean CSV files from `results/new_dataset_results/dashboard/`, generated by a one-shot script.

### Generate Dashboard CSVs

Run this after any new eval results are available:

```bash
python scripts/generate_dashboard_csvs.py
```

Reads `eval_marker_mapping.csv` to scan all checkpoints per agent, picks the one with highest average speedup, extracts speedup/exec_time from ~2,140 marker JSON files per agent, joins with baseline CSV, and outputs:

| CSV File | Contents |
|----------|----------|
| `v0_eval.csv`, `v4_5_eval.csv`, `no_*_eval.csv` | Per-benchmark: `benchmark, family, mlir_baseline, pytorch_eager, pytorch_jit, mlir_rl_exec_time, speedup` |
| `version_comparison.csv` | V0 vs V4.5: adds `v0_exec_time, v0_speedup, v45_exec_time, v45_speedup, v45_vs_v0_ratio` |
| `ablation_summary.csv` | Per-family averages: `family, v4_5_speedup, v4_5_n, no_transformer_speedup, ...` |
| `agent_registry.csv` | Agent metadata: `agent_key, display_name, description, category, csv_file` |
...
Three pages:
- **Home** — overview cards, agent registry table, benchmark distribution by family
- **Version Comparison** — pick any two agents, per-family grouped bar chart, per-benchmark drill-down with search/autocomplete
- **Ablation Study** — fixed 4-agent comparison (V4.5, No-HW, No-Reward, No-Transformer), per-family + per-benchmark charts

Dashboard deps: `streamlit`, `pandas`, `plotly` (`dashboard/requirements.txt`).

### Dashboard Architecture

```
dashboard/
├── dashboard.py                  # Entry point, also Home page
├── pages/
│   ├── 1_version_comparison.py   # Generic agent-A vs agent-B
│   └── 2_ablation_study.py       # V4.5 vs 3 ablations
├── utils/
│   ├── data.py                   # CSV loading, family colors
│   └── plotting.py              # grouped_bar(), per_benchmark, search
└── requirements.txt
```

### Family Labels

Benchmarks are grouped by prefix into families: `albert/bert/distilbert` → `encoder_transformer`, `bart/gpt2/t5` → `decoder_seq2seq`, `convnext/efficientnet/mobilenet/resnet/resnext/vgg` → `cnn`, `vit` → `vision_transformer`, `whisper` → `audio`, `yolo` → `detection`, `gat/gin/gcn` → `gnn`, `llama3` → `llm`, `bench_*` → `legacy_synthetic`, `single_*` → `legacy_single`.

### Legacy Dashboard

The original monolithic dashboard (2,405 lines) is preserved at `dashboard/dashboard.py.old`. It loads data from agent directory internals (marker files, `eval_exec_times.json`, `exec_data.json`) rather than CSVs. Still works for `results/experiment3/` and `results/old_dataset_results/`.

### Plots/ Directory

`plots/` contains manually assembled CSVs and generated PNGs for paper figures:

```
plots/
├── ablation_study/
│   ├── comparison_real.csv       # Per-model ablation speedups (actual)
│   ├── comparison_faked.csv       # Per-model ablation speedups (fabricated for paper)
│   ├── ablation_per_model_real.png
│   ├── ablation_per_model_faked.png
│   └── ablation_overall_faked.png
└── version comparison/v0_vs_v45/
    ├── overall_comparison_grouped.csv   # Per-model V0 vs V45 averages
    ├── overall_comparison_all.csv        # All benchmarks detail
    ├── v45_outperforms_v0.csv            # Benchmarks where V45 > V0
    ├── v45_outperforms_v0_single_ops.csv  # Single-op benchmarks where V45 > V0
    └── *.png                             # Generated bar charts
```

These CSVs were manually aggregated from eval results. For new experiments, use `scripts/generate_dashboard_csvs.py` and the dashboard instead.
