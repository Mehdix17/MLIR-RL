...
resolve_impl_config() {
    local impl="$1"
    # Strip rl_autoschedular_ prefix → v1, v2, v3, ...
    # Special-case: rl_autoschedular → baseline
    local token
...
with open('$BASE_CONFIG') as f:
    cfg = json.load(f)
impls = cfg.get('implementations', cfg.get('implementation', 'rl_autoschedular'))
if isinstance(impls, str):
    impls = [impls]
print(','.join(impls))
" 2>/dev/null || echo "rl_autoschedular")
else
...
if [[ -f "$BASE_JSON" ]]; then
    ...
    BASE_JOB=$(sbatch --job-name="base_${TIMESTAMP}" \
                      ...
                      scripts/get_base.sh "$BASE_CONFIG" 2>&1) || true
...
for impl in "${IMPLS[@]}"; do
    ...
    TRAIN_JOB=$(sbatch --job-name="train_${impl}" \
                       ...
                       scripts/train.sh "$IMPL_CONFIG" "$impl" 2>&1) || true
    ...
    if [[ "$TRAIN_JOB" =~ ^[0-9]+$ ]]; then
        ...
        EVAL_JOB=$(sbatch --job-name="eval_${impl}" \
                          ...
                          scripts/eval.sh "$IMPL_CONFIG" "$impl" 2>&1) || true
