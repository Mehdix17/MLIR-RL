...
# Evaluates all saved model checkpoints from an LSTM training run
#
...
# Default to latest LSTM run if not specified
if [ -z "$EVAL_DIR" ]; then
    # Find latest LSTM run
    LATEST_RUN=$(ls -dt ${PROJECT_ROOT}/results/lstm/run_* 2>/dev/null | head -1)
    if [ -z "$LATEST_RUN" ]; then
        echo "❌ No LSTM runs found in results/lstm/"
        exit 1
    fi
