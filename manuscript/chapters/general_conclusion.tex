...
Several directions emerge from this thesis. Short-term engineering improvements include extending the action space with additional transformation types such as loop unswitching and skewing, and integrating a learned cost model to replace static reward heuristics with execution-time predictions. Longer-term, the auto-scheduler should be extended to target GPU and accelerator backends, moving beyond the CPU-only setting of the current work.
...
Algorithmically, the AlphaZero-style planning explored by Tirichine et al.~\cite{tirichine2025reinforcement} remains a promising direction. The computational cost of Monte Carlo Tree Search rollouts currently limits its practicality, but hybrid approaches that use MCTS only for high-stakes decisions (e.g., tile size selection) while relying on the reactive PPO policy for routine transformations could combine the planning depth of search with the speed of learned policies. More broadly, the integration of the auto-scheduler as a pass within the MLIR compilation pipeline, rather than as an external tool, would lower the barrier to adoption and enable optimization of code produced by other MLIR-based compilers.
