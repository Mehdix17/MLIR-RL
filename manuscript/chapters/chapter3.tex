...
\textbf{Limitations.} The polyhedral model inherently limits program representation: loop bounds and array indices must be affine functions of enclosing iterators and symbolic constants. Because of this affine restriction, Tiramisu evaluates primarily on classical static kernels (e.g., highly regular scientific loops), lacking structural exposure to the dynamic tensor shapes and indirect memory accesses dominant in real-world neural network optimization. Moreover, the state encoders often rely on sequential recurrent units (e.g., LSTMs or feedforward networks). This recursive, sequential processing structurally struggles to connect non-adjacent, global dependencies nested deep within compiler subgraphs, rendering it ill-equipped for deeply nested topologies where an inner loop geometry intimately depends on an arbitrary outer loop.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l X X X X}
        \toprule
        \textbf{System} & \textbf{Optimization Paradigm} & \textbf{State Abstraction} & \textbf{Credit Assignment} & \textbf{HW Adaptability} \\
        \midrule
        Halide + DRL~\cite{pecenin2019halide}
        & Schedule parameter search
        & Flat loop features
        ...
        & \textbf{HW-aware subgraph sched.}
        & \textbf{Global \gls{ast} attention}
        & \textbf{Shaped (Arith. Intensity)}
        & \textbf{Explicit (Embeddings)} \\
        \bottomrule
    \end{tabularx}
\end{table}
