...
An earlier effort by Hennouni and El Hassane~\cite{henouni2022utilisation} had explored \gls{rl} for Tiramisu using a feedforward policy network with a fully observed \gls{mdp}, where the entire state was visible at every step. Lamouri and Merad improved on this by adopting a partially observable \gls{mdp} (POMDP) formulation with an \gls{lstm} encoder, which allowed the agent to accumulate knowledge about the program structure across multiple observation steps.
...
The reward function is the log-speedup relative to the unoptimized baseline execution time. Penalties are applied for invalid transformations (e.g., tile sizes that do not divide loop bounds) and for execution failures during the compilation and \gls{jit} execution pipeline.
...
To encode this tree-structured observation into a fixed-size vector suitable for the policy network, the authors introduce a recursive \gls{lstm} encoder. Beginning at the leaf nodes, the features of each operation are embedded by a multi-layer perceptron. The embeddings of consumer operations are then combined with the embeddings of their producer operations through an \gls{lstm} that processes the AST bottom-up, producing an aggregated embedding at the root that summarizes the entire computation graph. This recursive structure naturally captures the dependence chain: the encoding of a consumer reflects not only its own features but also those of all operations whose output it consumes.
...
Despite these advances, the authors identify several limitations that persist in their system. The observation remains hardware-agnostic: the agent must infer optimal tile sizes and parallelization strategies through trial and error rather than conditioning on known cache and core specifications. The reward function remains entirely sparse, with no intermediate signal guiding the agent toward promising states during an episode. The recursive \gls{lstm} encoder processes loop levels sequentially, which may struggle with deeply nested structures where the relationship between distant levels is not well captured. The action space, while expanded with fusion and improved interchange mechanisms, remains limited to six transformation types, missing operations such as padding, packing, and unrolling that could further improve memory alignment and instruction-level parallelism. These limitations define the research space addressed by the present work.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{1.5cm}}
        \toprule
        \textbf{System} & \textbf{\gls{ir} / Compiler} & \textbf{State Representation} & \textbf{Action Space} & \textbf{Reward} & \textbf{Hardware-Aware} \\
        \midrule
        Halide + DRL~\cite{pecenin2019halide}
        & Halide IR
        & Loop nest features, buffer sizes, partial schedule
        ...
        & Polyhedral IR
        & Polyhedral features (iteration domains, dependence vectors)
        ...
        & \gls{llvm} IR
        & Aggregate program statistics (inst counts, CFG properties)
        & Pass selection, compiler flags
        ...
        & 6 actions (same as Tirichine)
        & Log-speedup (sparse)
        & \textbf{Yes} \\
        \addlinespace
        \textbf{This work (V2)}
        & \gls{mlir} Linalg
        & \gls{ast} + \gls{lstm} (same as Tirichine)
        & 6 actions (same as Tirichine)
...
\begin{enumerate}
    ...
    \item \textbf{Restricted action diversity.} The action space of prior \gls{mlir}-based systems is limited to six transformation types. Important transformations for memory layout optimization (padding, which aligns data for efficient vectorization, and packing, which reorganizes data for better cache utilization) and for instruction-level parallelism (unrolling, which reduces loop overhead and exposes independent instructions) are absent. Expanding the action space gives the agent finer-grained control over the optimization process.

    \item \textbf{Evaluation fragmentation.} Most prior work evaluates on a single benchmark suite or a small set of programs, often without standardized evaluation protocols or multi-configuration comparisons. This makes it difficult to isolate the contribution of individual design choices and to compare results across systems. A systematic evaluation framework with shared datasets, consistent training protocols, and controlled version comparisons is needed to advance the field.
\end{enumerate}

The contributions of this thesis directly address each of these challenges. The hardware-aware observation (V1), shaped reward function (V2), transformer-based loop nest encoder (V3), and expanded action space with pad, pack, and unroll transformations (V4) are described in detail in Chapter~4. The unified V4 system combines all of these improvements to evaluate their cumulative effect on optimization performance.
...
These gaps directly motivate the design decisions in our system. The next chapter presents the design and implementation of our \gls{rl}-based auto-scheduler, describing the observation space, action space, reward function, and policy architecture for each of the four versions that address these challenges.
