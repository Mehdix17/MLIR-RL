...
\begin{itemize}
\item \textbf{\gls{ast}-based state representation and recursive \gls{lstm} encoder :} A central contribution of Tirichine et al. is the replacement of the flat per-operation feature vector with a structured \gls{ast}-based state representation. The computation graph is represented as a tree where leaf nodes correspond to individual Linalg operations and internal nodes encode the hierarchical structure of producer-consumer relationships. Each operation node carries the same features as in the Bendib baseline (operation type, loop bounds, iterator types, access matrices).
...
\item \textbf{Fusion action and producer-consumer traversal :} To exploit the producer-consumer relationships captured by the \gls{ast}, Tirichine et al. introduce a new Fusion action \gls{tf}. This action merges a producer operation into its consumer's loop nest using \gls{mlir}'s tiling and fusion mechanisms, eliminating the intermediate buffer that would otherwise store the producer's output. When combined with tiling and parallelization, the action becomes \gls{tf}, which tiles both operations, fuses them, and parallelizes the resulting loop nest.
...
\item \textbf{Domain generalization: neural networks and \gls{lqcd}.} A distinctive feature of the Tirichine system is its demonstration of generalization across two fundamentally different computational domains. The first domain consists of neural network operators common in deep learning: matrix multiplications, 2D convolutions, element-wise operations, and their compositions into blocks and full models (VGG-16~\cite{simonyan2015vgg}, ResNet~\cite{he2016resnet}, MobileNet~\cite{howard2019mobilenetv3}). The second domain is \gls{lqcd}, a field of computational physics involving stencil-like computations on four-dimensional space-time grids.
...
\item \textbf{AlphaZero adaptation :} As an alternative to \gls{ppo}, the authors explore the AlphaZero algorithm~\cite{silver2017alphazero} for the code optimization problem. AlphaZero uses  \gls{mcts} guided by a neural network to plan action sequences, potentially enabling deeper lookahead than the reactive policy of \gls{ppo}. In the adaptation, the \gls{mcts} simulates sequences of loop transformations from the current program state, with a learned value network evaluating partial sequences and a policy network proposing promising actions. The AlphaZero-based agent is compared against the \gls{ppo} baseline, revealing that while AlphaZero can discover higher-quality schedules in some settings, the computational cost of \gls{mcts} rollouts makes it less practical for large-scale training.

\item \textbf{Results and limitations :} The evaluation conducted by Tirichine et al. covers both single operations (various matrix multiplication and convolution sizes) and full neural network models (VGG-16~\cite{simonyan2015vgg}, ResNet~\cite{he2016resnet}, MobileNet~\cite{howard2019mobilenetv3}). On single operations, the \gls{rl} agent achieved speedups competitive with hand-tuned schedules. On full models, the agent demonstrated notable performance, including a 2.5$\times$ speedup on MobileNe relative to standard PyTorch execution, though it underperformed on architectures with highly regular patterns (VGG-16~\cite{simonyan2015vgg}, 1.09$\times$) and on ResNet~\cite{he2016resnet} (2.1$\times$ compared to PyTorch's 4.25$\times$).
\end{itemize}
...
Beyond the \gls{mlir} ecosystem, learning-based optimization approaches have been explored in other polyhedral and domain-specific compilers. Hennouni and El Hassane~\cite{henouni2022utilisation} investigated the use of reinforcement learning for exploring the combinatorial search space of transformations in the Tiramisu compiler framework. Specifically, they modeled the selection and ordering of polyhedral schedules—such as loop tiling, interchange, and unrolling—as a sequential decision-making problem. By training an \gls{rl} agent to navigate these valid transformation sequences, their approach aimed to discover optimized schedules that minimize code execution time, leveraging empirical speedups to formulate the reward function. While their work successfully demonstrated the viability of \gls{rl}-guided optimization for polyhedral code generation, it was heavily constrained to single-level loop nests and did not scale to handle full computation graphs featuring complex producer-consumer dependencies.
