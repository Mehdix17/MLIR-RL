...
In the \gls{mlir}-\gls{rl} framework established in prior work~\cite{bendib2024reinforcement}, this optimization is represented by the \gls{tp} action or the composite \gls{tpf} action.
...
In \gls{mlir}-\gls{rl}, this multi-operation optimization is mapped directly to the \gls{f} action or combined into the multi-tier \gls{tpf} action. The structural contraction of separate loop bodies into a localized execution pipeline is presented in Figure~\ref{fig:fusion-before}.
