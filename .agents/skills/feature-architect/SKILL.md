...
- **Slurm/HPC deployment**: every feature is deployed via Slurm (`scripts/train/train.sh`, `scripts/eval/eval.sh`). The design doc must include `--cpus-per-task`, `--mem`, `--partition`, and `--time` requirements. See `docs/hpc/HPC_HARDWARE.md` for available partitions and `docs/design/done/v5_training_acceleration.md` for current resource usage and the decided V5 resource matrix.
