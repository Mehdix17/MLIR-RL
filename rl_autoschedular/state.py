...
def extract_bench_features_from_file(bench_name: str, file_path: str, root_execution_time: int):
    ...
    Returns:
        BenchmarkFeatures: the extracted benchmark features
    """
    ast_dumper_path = os.getenv("AST_DUMPER_BIN_PATH")
    
    if not ast_dumper_path:
        raise RuntimeError(
            "AST_DUMPER_BIN_PATH environment variable is not set. "
            "Please set it to the path of the AstDumper binary, e.g.:\n"
            "export AST_DUMPER_BIN_PATH=/scratch/mb10856/MLIR-RL/tools/ast_dumper/build/bin/AstDumper"
        )
    
    if not os.path.exists(ast_dumper_path):
        raise RuntimeError(
            f"AST dumper binary not found at: {ast_dumper_path}\n"
            f"Please check the AST_DUMPER_BIN_PATH environment variable."
        )
    
    result = subprocess.run(
        [ast_dumper_path, file_path],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE
    )
    raw_ast_info = result.stdout.decode('utf-8')
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"AST dumper failed for benchmark '{bench_name}' (file: {file_path})\n"
            f"Return code: {result.returncode}\n"
            f"Stderr: {stderr_output}\n"
            f"Stdout: {raw_ast_info[:500]}"
        )
    
    if stderr_output:
        print_error(f"AST dumper warnings for '{bench_name}':\n{stderr_output}")
...
def __extract_bench_features_from_ast_result(bench_name: str, raw_ast_info: str, root_execution_time: int):
    ...
    # Check if AST dumper produced valid output
    if "########################################" not in raw_ast_info:
        raise ValueError(
            f"AST dumper output is invalid for benchmark '{bench_name}'. "
            f"Expected separator '########################################' not found.\n"
            f"AST dumper output (first 500 chars):\n{raw_ast_info[:500]}\n"
            f"Check that AST_DUMPER_BIN_PATH is set correctly and the binary works: {os.getenv('AST_DUMPER_BIN_PATH')}"
        )
