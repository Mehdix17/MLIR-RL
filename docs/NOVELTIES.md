...
### Novelty 3: Expanded Transformation Action Space (V4)
...
**The Solution:** Implement finer-grained transformations available in the MLIR Transform Dialect. This novelty is implemented in **`rl_autoschedular_v4`**:
...
See [`docs/Novelties/v4_action_space_expansion.md`](Novelties/v4_action_space_expansion.md) for full implementation details.
...
### Phase 2: Action Space Expansion (Weeks 5-8) — V4 Implemented
* **Task 2.1:** Integrate **Pad**, **Pack**, and **Unroll** into `rl_autoschedular_v4/actions/` and `rl_autoschedular_v4/transforms.py`.
...
3.  **Efficiency gains:** Through Pad, Pack, and Unroll actions (V4) not present in the baseline, enabling finer-grained control over memory layout and instruction-level parallelism.
