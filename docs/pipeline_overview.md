...
**`data/nn/`** layout:
```
code_files/          ← final wrapped .mlir files (the ones train.py reads)
generated/
...
```
There are currently **no JSON files** in `data/nn/` — that JSON (mapping bench names → baseline times) must be created by running `get_base.py` or `scripts/build_generated_exec_json.py`.
...
| `train_generated.sh` | Slurm job — runs `train.py` with `config/generated-benchmarks.json` |
| `eval_generated.sh` | Slurm job — runs evaluation against a saved checkpoint |
| `build_generated_exec_json.py` | Measures baseline execution time for each `.mlir` in a folder → writes the JSON needed by `train.py` |
| `train_example.sh` | Example train job with custom config |

**`results/run_N/`** — one folder per training run:
...
the MLIR, but `train.py` can't use it yet. The full chain is:
...
Step 3 ❌  get_base.py  (runs each .mlir, measures baseline ns)
...
3. Run `get_base.py` on `data/nn/code_files/` to produce `base_exec_times.json`
